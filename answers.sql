...
SELECT models.name AS model_name, brands.name AS brand_name, headquarters FROM brands, models WHERE models.name='Mustang' AND brands.name='Ford';
...
SELECT * FROM models WHERE model_id BETWEEN 15 AND 24;
...
SELECT models.year, models.name, model_id, awards.name AS name FROM models LEFT JOIN awards ON models.model_id=awards.winner_id WHERE models.year>='1964';
...
SELECT b.name, m.year, m.name FROM models AS m INNER JOIN brands AS b ON b.brand_id=m.brand_id ORDER BY b.name;
...
SELECT brands.name, brands.founded FROM brands WHERE brands.brand_id NOT IN (SELECT models.brand_id FROM models);
